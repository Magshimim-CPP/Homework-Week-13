			...
			if (Helper::getMessageTypeCode(clientSocket) == MT_CLIENT_UPDATE)
			{
				...
				if (nameLen > 0) //if the user want's an update of some sort of a chat with someone.
				{
					...
					if (msgLen > 0) //if the user want's to send a message to someone.
					{
						...
						MessageStructure newMsg; //creating a new MessageStructure for the message.
						newMsg.sender = thisUser;
						newMsg.receiver = otherUser;
					...
					else //if the client want's an update on a chat with someone without sending a message.
					{
						...
						chatFile.open(path); //opening the file related to the current chat's path
...
void Server::messagesHandler()
{
	...
	while (true) //infinite loop.
	{
		...
		addToChatFile.open(path, ios_base::app); //opening the file related to the current chat's path for writing/creation. (ios_base::app - to keep current file data)
		...
		file.open(path); //opening the file related to the current chat's path
		...
		Helper::send_update_message_to_client(_usersOnline.find(msg.sender)->second, fileContent.str(), msg.receiver, allUsersStringByProtocol());
	}
}
